...
def test_fecha_ddmmaaaa_desde_datetime():
    assert acc.fecha_ddmmaaaa_desde_valor(datetime(2026, 7, 15)) == "15-07-2026"


def test_fecha_ddmmaaaa_desde_string_dd_mm_yyyy_con_barras():
    assert acc.fecha_ddmmaaaa_desde_valor("15/07/2026") == "15-07-2026"


def test_fecha_ddmmaaaa_desde_string_dd_mm_yyyy_con_guiones():
    assert acc.fecha_ddmmaaaa_desde_valor("15-07-2026") == "15-07-2026"


def test_fecha_ddmmaaaa_desde_string_invalida_se_sanitiza():
    assert acc.fecha_ddmmaaaa_desde_valor("fecha rara") == "fecha_rara"


def test_nombre_esperado_archivo_heic_pasa_a_jpg():
    nombre = acc.nombre_esperado_archivo("UMAG-001", "Shell", "15/07/2026", ".HEIC")
    assert nombre == "UMAG-001_Shell_15-07-2026.jpg"
...
def test_nombre_esperado_archivo_normaliza_extension_a_minusculas():
    ...
    assert nombre == "CFLI-004_Beckman_10-05-2026.pdf"
...
def test_planificar_renombrado_fila_ya_correcto(tmp_path, monkeypatch):
    ...
    (tmp_path / "UMAG").mkdir()
    (tmp_path / "UMAG" / "UMAG-001_Shell_15-07-2026.jpg").write_bytes(b"contenido")

    fila = {
        "fila": 2, "n_ref": "UMAG-001",
        "archivo_origen": "UMAG\\UMAG-001_Shell_15-07-2026.jpg",
        "proveedor_tag": "Shell", "fecha": "15/07/2026",
    }
    item = acc.planificar_renombrado_fila(fila, {})
    assert item["accion"] == "ya_correcto"
    assert item["fila"] == 2
    assert item["nombre_nuevo"] == "UMAG-001_Shell_15-07-2026.jpg"
...
def test_planificar_renombrado_fila_renombrar_sin_conversion(tmp_path, monkeypatch):
    ...
    assert item["nombre_nuevo"] == "CFLI-003_Beckman_01-02-2026.pdf"
    assert item["ruta_actual"] == tmp_path / "CFLI" / "factura_original.pdf"
    assert item["ruta_nueva"] == tmp_path / "CFLI" / "CFLI-003_Beckman_01-02-2026.pdf"
...
def test_planificar_renombrado_fila_convertir_heic(tmp_path, monkeypatch):
    ...
    assert item["nombre_nuevo"] == "UMAG-025_Anwo_20-03-2026.jpg"
...
def test_planificar_renombrados_procesa_varias_filas(tmp_path, monkeypatch):
    ...
    (tmp_path / "UMAG" / "UMAG-002_Shell_01-01-2026.jpg").write_bytes(b"x")

    filas = [
        {"fila": 2, "n_ref": "UMAG-001", "archivo_origen": "UMAG\\a.jpg",
         "proveedor_tag": "Shell", "fecha": "01/01/2026"},
        {"fila": 3, "n_ref": "UMAG-002", "archivo_origen": "UMAG\\UMAG-002_Shell_01-01-2026.jpg",
         "proveedor_tag": "Shell", "fecha": "01/01/2026"},
    ]
...
def test_ejecutar_plan_renombrado_renombra_sin_convertir(tmp_path):
    ...
    ruta_nueva = tmp_path / "CFLI-003_Beckman_01-02-2026.pdf"
...
def test_ejecutar_plan_renombrado_convierte_heic_y_borra_original(tmp_path):
    ...
    ruta_nueva = tmp_path / "UMAG-025_Anwo_20-03-2026.jpg"
...
def test_ejecutar_plan_renombrado_ya_correcto_no_hace_nada(tmp_path):
    ruta = tmp_path / "UMAG-001_Shell_15-07-2026.jpg"
...
def test_aplicar_renombrados_renombra_y_actualiza_master(tmp_path, monkeypatch):
    ...
    assert advertencias == []
    assert ws.cell(row=5, column=15).value == "CFLI\\CFLI-003_Beckman_01-02-2026.pdf"
    assert ws.cell(row=5, column=16).value is not None
    assert (tmp_path / "CFLI" / "CFLI-003_Beckman_01-02-2026.pdf").exists()
...
def test_aplicar_renombrados_ya_correcto_no_cuenta_ni_toca_master(tmp_path, monkeypatch):
    ...
    ws = _WsFake()
    filas = [{"fila": 2, "n_ref": "UMAG-001", "archivo_origen": "UMAG\\UMAG-001_Shell_15-07-2026.jpg",
              "proveedor_tag": "Shell", "fecha": "15/07/2026"}]
